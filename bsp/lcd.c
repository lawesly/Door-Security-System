...
{
  POINT_COLOR = BLACK;
  LCD_ShowString(20,0,200,24,24,"Smart Home Center");//显示标题
  LCD_DrawLine(0, 24, 240, 24);//第一条横线
  LCD_ShowString(2,25,200,16,16," Current Time       Mode");//显示时间标题
  LCD_ShowString(lcddev.width-95,58,200,24,24," Home ");    //显示系统模式
  LCD_DrawLine(0, 42, 240, 42);//画第二条横线
  LCD_DrawLine(0, 95, 240, 95);//画第三条横线
  LCD_DrawLine(0, 207, 240, 207);//画第四条横线
  LCD_DrawLine(120, 24, 120, 95);//画第1条竖线
  LCD_ShowString(0,96,200,24,24,"   Sensor Status");//传感器状态标题
  LCD_DrawLine(0, 122, 240,122);//画第五条横线
  LCD_ShowString(0,124,200,16,16,"Temperature:");	
  LCD_ShowString(0,142,200,16,16,"Humidity   :");
  LCD_ShowString(0,158,200,16,16,"Fire Status:"); 
  LCD_ShowString(0,176,200,16,16,"M         Q:");
  LCD_ShowString(0,209,200,24,24,"   Control Pannel");//控制面板标题
  LCD_DrawLine(0, 234, 240, 234);//画第六条横线	
  ...
  LCD_ShowString(0,290,200,16,16,"Curtain:  OPEN  CLOSE  K/T");
  return 0;
}
//INIT_BOARD_EXPORT(Draw_window);
...
void Draw_Number_Pad(void)
{
  ...
  LCD_ShowString(38,178-8,200,24,24,"0");
  LCD_ShowString(116,178-8,200,24,24,"1");	
  LCD_ShowString(199,178-8,200,24,24,"2");
  LCD_ShowString(38,221-8,200,24,24,"3");
  LCD_ShowString(116,221-8,200,24,24,"4");
  LCD_ShowString(199,221-8,200,24,24,"5");	
  LCD_ShowString(38,257-8,200,24,24,"6");
  LCD_ShowString(116,257-8,200,24,24,"7");
  LCD_ShowString(199,257-8,200,24,24,"8");
  LCD_ShowString(38,297-8,200,24,24,"9");	
  LCD_ShowString(116,297-8,200,24,24,"C");
...
void security_door_window(void)
{
	...
	LCD_ShowString(0,10,240,24,24,"                    ");	
    LCD_ShowString(0,10,240,24,24,"Door Security System");//显示标题
	LCD_ShowString( 0, 80,240,240,16 ,"The Card ID is:                 ");
	LCD_DrawLine(0, 48, 240, 48);//第一条横线
	LCD_DrawLine(0, 272, 240, 272);//第二条横线
	LCD_ShowString(15,282,200,24,24,"                  ");
	LCD_ShowString(15,282,200,24,24,"Register   Delete ");
	LCD_DrawLine(120, 272, 120, 320);//画一条竖线
}
...
void register_delete_GUI(uint8_t mode)
{
	...
	if(mode)
	{
		...
		POINT_COLOR = RED;
		LCD_ShowString(0,10,240,24,24,"  Register New Tag  ");//显示标题
		POINT_COLOR = BLACK;
	...
	else
	{
		...
		HAL_NVIC_DisableIRQ(RTC_IRQn);
		POINT_COLOR = RED;
		LCD_ShowString(0,10,240,24,24,"    Delete  Tags    ");//显示标题
		POINT_COLOR = BLACK;
		HAL_NVIC_EnableIRQ(RTC_IRQn);
		Show_BDN
	}
